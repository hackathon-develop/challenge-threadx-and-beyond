...
UINT mqtt_client_connect_subscribe(NX_IP* ip_ptr, NX_PACKET_POOL* pool_ptr)
{
    ...
    nxd_mqtt_client_connect(&mqtt_client, &server_ip, NXD_MQTT_PORT, MQTT_KEEP_ALIVE_TIMER, 0, NX_WAIT_FOREVER);

    /* Subscribe to the topic with QoS level 0. */
    nxd_mqtt_client_subscribe(&mqtt_client, TOPIC_NAME, STRLEN(TOPIC_NAME), QOS0);
...
void mqtt_client_publish(char* message_string)
{
    ...
    nxd_mqtt_client_publish(
        &mqtt_client, TOPIC_NAME, STRLEN(TOPIC_NAME), (CHAR*)message_string, 30, 0, QOS1, NX_WAIT_FOREVER);
